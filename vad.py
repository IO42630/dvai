...
class VADAudio(Audio):
    ...
    def vad_collector(self, padding_ms = 300, ratio = 0.75, frames = None):
        ...
        separated by yieldinga single None.
...
if __name__ == '__main__':
    ...
    N_FEATURES = 26
    N_CONTEXT = 9
    DEFAULT_MODEL = "/home/user/app/deepspeech/deepspeech-models/deepspeech-0.5.1-models/"
    DEFAULT_DEVICE = 9  # speedlink

    import argparse
    parser = argparse.ArgumentParser(description = "Stream from microphone to DeepSpeech using VAD")

    help_ = "Set aggressiveness of VAD: an integer between 0 and 3, "
    help_ += "0 being the least aggressive about filtering out non-speech, "
    help_ += "3 the most aggressive. Default: 3"
    ...
    help_ = "Path to the model (protocol buffer binary file, "
    help_ += "or entire directory containing all standard-named files for model)"
    ...
    parser.add_argument('-a',
                        '--alphabet',
                        default = 'alphabet.txt',
                        help = "Path to the configuration file specifying the alphabet used by the network. Default: "
                               "alphabet.txt")
    ...
    help_ = "Device input index (Int) as listed by pyaudio.PyAudio.get_device_info_by_index(). "
    help_ += "If not provided, falls back to PyAudio.get_default_device()"
    ...
    parser.add_argument('-r', '--rate',
                        ...
                        help = f"Input device sample rate. Default: {DEFAULT_SAMPLE_RATE}. Your device may require "
                               f"44100.")

    parser.add_argument('-nf', '--n_features',
                        type = int, default = N_FEATURES,
                        help = f"Number of MFCC features to use. Default: {N_FEATURES}")
    parser.add_argument('-nc', '--n_context',
                        type = int, default = N_CONTEXT,
                        help = f"Size of the context window used for producing timesteps in the input vector. "
                               f"Default: {N_CONTEXT}")
    help_ = "The alpha hyperparameter of the CTC decoder. Language Model weight."
    help_ += f"Default: {LM_ALPHA}"

    parser.add_argument('-la', '--lm_alpha',
                        type = float,
                        default = LM_ALPHA,
                        help = help_)
    parser.add_argument('-lb', '--lm_beta',
                        type = float,
                        default = LM_BETA,
                        help = f"The beta hyperparameter of the CTC decoder. Word insertion bonus. Default: {LM_BETA}")
    parser.add_argument('-bw', '--beam_width',
                        type = int,
                        default = BEAM_WIDTH,
                        help = f"Beam width used in the CTC decoder when building candidate transcriptions. Default: "
                               f"{BEAM_WIDTH}")
